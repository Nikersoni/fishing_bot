...
FISH_TYPES = {"карп": (1,5), "щука": (2,6), "сом": (3,8), "окунь": (1,4)}
...
def register(bot, db, save_db):
    ...
    def fish_handler(message):
        ...
        if user["items"].get("удочка",0)<1 or user["items"].get("приманка",0)<1:
            bot.send_message(message.chat.id, "🎣 Нет удочки или приманки!")
            return
        ...
        save_db()
        bot.send_message(message.chat.id, f"🎣 Вы поймали {amount} {fish}!")
